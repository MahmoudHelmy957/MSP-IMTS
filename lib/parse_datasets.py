		...
		if use_ms:
			...
			def collate_fn_ms(batch):
				...
				for rid, tt_rel, vals, mask, t_bias in batch:
					...
					# tt_rel can sometimes be empty => guard it
					if tt_rel.numel() > 0:
				...
				# 🔴 IMPORTANT: make time_max a TENSOR (like before)
				local_time_max_tensor = torch.tensor(
					local_time_max,
					device=device,
					dtype=time_max.dtype      # use same dtype as original time_max
				)

				return patch_variable_time_collate_fn_ms(
					batch4, args, device=device,
					data_min=data_min, data_max=data_max,
					time_max=local_time_max_tensor,          # <- tensor, not float
					scales_hours=scales, strides_hours=strides,
