...
def parse_datasets(args, patch_ts=False, length_stat=False):
	...
	### Activity dataset ###
	    ...
	    # === Load dataset ===
	    total_dataset = PersonActivity('../data/PersonActivity/', n_samples=args.n, download=False, device=device)
	    ...
	    print("Dataset n_samples:", len(total_dataset), len(train_data), len(val_data), len(test_data))
	    test_record_ids = [record_id for record_id, tt, vals, mask in test_data]
	    print("Test record ids (first 5):", test_record_ids[:5])
	    print("Test record ids (last 5):", test_record_ids[-5:])
	    ...
	    # === Chunk sequences into sliding windows ===
	    ...
	    use_ms = hasattr(args, "multi_scales") and args.multi_scales not in (None, "", [])
	    if use_ms:
	        from lib.person_activity import patch_variable_time_collate_fn_ms_activity
	        import re
	        ...
	        assert len(scales_ms) == len(strides_ms), "Mismatch in multi_scales and multi_strides"

	        print(f"[Multi-scale] scales={scales_ms}, strides={strides_ms}")
	        collate_fn = lambda batch: patch_variable_time_collate_fn_ms_activity(
	            batch, args, device=device,
	            data_min=data_min, data_max=data_max, time_max=time_max,
	            scales_ms=scales_ms, strides_ms=strides_ms, history_ms=float(args.history)
	        )
	    else:
	        # single-scale fallback
	        collate_fn = patch_variable_time_collate_fn if patch_ts else variable_time_collate_fn
	    ...
	    # === Optional: Sequence length stats ===
	    if length_stat:
