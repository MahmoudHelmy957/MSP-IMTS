...
def parse_datasets(args, patch_ts=False, length_stat=False):
	...
	### USHCN dataset ###
		...
		# Load full dataset: list of (record_id, tt, vals, mask)
		...
		record_id, tt, vals, mask = train_data[0]
		input_dim = vals.size(-1)
		data_min, data_max, time_max = get_data_min_max(seen_data, device)  # (n_dim,), (n_dim,), scalar

		# ---------- choose collate (MS vs single-scale) ----------
		use_ms = hasattr(args, "multi_scales") and args.multi_scales not in (None, "", [])
		print(f"use_ms={use_ms} fusion={args.fusion} "
			f"scales={args.multi_scales or 'single-scale'} "
			f"strides={args.multi_strides or 'same-as-scales'}")

		# Chunk windows into (record_id, local_tt, vals, mask, t_bias)
		...
		if use_ms:
			# Multi-scale path (months). Parse CLI lists.
			from lib.ushcn import USHCN_patch_variable_time_collate_fn_ms
			import re as _re
			scales_months = [float(x) for x in _re.split(r"[,\s]+", args.multi_scales.strip()) if x]
			if getattr(args, "multi_strides", None) in (None, "", []):
				strides_months = scales_months[:]
			else:
				strides_months = [float(x) for x in _re.split(r"[,\s]+", args.multi_strides.strip()) if x]
				assert len(scales_months) == len(strides_months), "multi_scales and multi_strides length mismatch"

			collate_fn_ms = lambda batch: USHCN_patch_variable_time_collate_fn_ms(
				batch, args, device=device,
				data_min=data_min, data_max=data_max, time_max=time_max,
				scales_months=scales_months, strides_months=strides_months,
				history_months=float(args.history)
			)
