# lib/plot_train_val_loss.py
# Parse train.log and plot Train vs Val loss (single run or mean±std across runs)
...
# OLD:
# INFO | Train - Loss (last batch): 0.00095
# NEW:
# INFO | Train - Loss (last batch, scaled): 0.006650
TRAIN_LOSS_RE = re.compile(
    rf"Train\s*-\s*Loss\s*\(last batch(?:\s*,\s*scaled)?\)\s*:\s*(?P<loss>{NUM})"
)

# OLD:
# INFO | Val - Loss, MSE, MAE: 0.00093, 0.00093, 0.01801
VAL_LINE_RE_OLD = re.compile(
    rf"Val\s*-\s*Loss,\s*MSE,\s*MAE\s*:\s*"
    rf"(?P<val_loss>{NUM})\s*,\s*(?P<val_mse>{NUM})\s*,\s*(?P<val_mae>{NUM})"
)

# NEW:
# INFO | Val - GLOBAL masked MSE scaled: 0.006209 | real: 0.427934 | real MAE: 0.545934
VAL_LINE_RE_NEW = re.compile(
    rf"Val\s*-\s*GLOBAL masked MSE scaled:\s*(?P<mse_scaled>{NUM})\s*\|\s*"
    rf"real:\s*(?P<mse_real>{NUM})\s*\|\s*real MAE:\s*(?P<mae_real>{NUM})"
)
...
def parse_log_file(path: str) -> Dict[str, List[dict]]:
    ...
    Returns:
        runs[expid] = list of records, each record:
            {
              "epoch": int,
              "train_loss": float or np.nan,
              "val_loss": float or np.nan,   # for NEW format: this is mse_scaled
              "val_mse": float or np.nan,    # for NEW format: this is mse_real
              "val_mae": float or np.nan     # for NEW format: this is mae_real
            }
    """
    ...
    def _flush_if_ready():
        nonlocal current_expid, current_epoch, current_train_loss, current_val
        if current_expid is None or current_epoch is None:
            return

        rec = {
            "epoch": int(current_epoch),
            "train_loss": float(current_train_loss) if current_train_loss is not None else np.nan,
    ...
    for line in lines:
        ...
        m_tr = TRAIN_LOSS_RE.search(line)
        if m_tr and current_epoch is not None:
            current_train_loss = float(m_tr.group("loss"))
            continue

        # NEW val format
        m_val_new = VAL_LINE_RE_NEW.search(line)
        if m_val_new and current_epoch is not None:
            mse_scaled = float(m_val_new.group("mse_scaled"))
            mse_real = float(m_val_new.group("mse_real"))
            mae_real = float(m_val_new.group("mae_real"))
            # For plotting Train vs Val on same scale: use scaled metric as "val_loss"
            current_val = (mse_scaled, mse_real, mae_real)
            continue

        # OLD val format
        m_val_old = VAL_LINE_RE_OLD.search(line)
        if m_val_old and current_epoch is not None:
            current_val = (
                float(m_val_old.group("val_loss")),
                float(m_val_old.group("val_mse")),
                float(m_val_old.group("val_mae")),
            )
            continue
...
def plot_mean_over_runs(
    ...
    epochs, train_mat = _align_runs_for_mean(records_by_run, "train_loss")
    _, val_mat = _align_runs_for_mean(records_by_run, "val_loss")

    train_mean = np.nanmean(train_mat, axis=0)
    train_std = np.nanstd(train_mat, axis=0, ddof=1) if train_mat.shape[0] > 1 else np.zeros_like(train_mean)
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Loss / Scaled MSE")
    plt.title(f"{title} | mean ± std over {train_mat.shape[0]} Seeds")
    plt.legend()
    plt.grid(True, alpha=0.3)
...
def main():
    ap = argparse.ArgumentParser("Plot Train/Val loss from train.log files")
    ...
    ap.add_argument("--title", type=str, default="Train vs Val Loss", help="Plot title prefix.")
