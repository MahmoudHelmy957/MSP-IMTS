...
# + CHANGED: Multi-Scale (MS) TRAIN/VAL/TEST loss is now **per-dimensional (channel-balanced) masked MSE**
#   (same philosophy as compute_error(..., reduce="mean") in lib.utils).
#
# Notes:
#  - MS path now optimizes: mean over channels of (masked MSE per channel).
#  - SS path still uses lib.utils.compute_all_losses/evaluation (already channel-balanced in your code).
...
# ------------------------- per-dimensional masked metrics (channel-balanced) -------------------------
def _masked_metrics_perdim(pred, tgt, msk):
    """
    pred,tgt,msk: (B,L,C)
    Returns channel-balanced metrics:
      mse = mean_c( mean( (e^2) over masked points of channel c ) )
      mae = mean_c( mean( |e| over masked points of channel c ) )
    """
    mb = msk.bool() if msk.dtype == torch.bool else (msk > 0.5)
    if mb.sum().item() == 0:
    ...
    for c in range(C):
        ...
        tgt_abs = tgt[..., c][mb_c].abs()
        mape_c = (d.abs() / torch.clamp(tgt_abs, min=1e-8)).mean()
    ...
    mse = torch.stack(mse_list).mean().item()
    mae = torch.stack(mae_list).mean().item()
    mape = torch.stack(mape_list).mean().item()
...
def _masked_metrics_per_channel(pred, tgt, msk):
    ...
    NOTE: overall here is GLOBAL (weighted by all masked points), per_ch is per-channel.
    """
    mb = msk.bool() if msk.dtype == torch.bool else (msk > 0.5)
    if mb.sum().item() == 0:
...
# ------------------------- loss (shared): channel-balanced masked MSE -------------------------
def _loss_channel_balanced_mse(pred, tgt, msk):
    """
    pred,tgt,msk: (B,L,C)
    loss = mean_c ( sum((pred-tgt)^2 * mask) / sum(mask) )
    """
    mb = msk.bool() if msk.dtype == torch.bool else (msk > 0.5)
    if mb.sum().item() == 0:
    ...
    diff2 = (pred - tgt) ** 2
    se_sum = (diff2 * mb).sum(dim=(0, 1))  # (C,)
    cnt = mb.sum(dim=(0, 1))  # (C,)
    avai = cnt > 0
    if avai.sum().item() == 0:
        return None
    loss = (se_sum[avai] / (cnt[avai] + 1e-8)).mean()
    return loss
...
if __name__ == "__main__":
    ...
    run_logger.info("LOSS_MODE | MS path uses PER-DIMENSION (channel-balanced) masked MSE")
    ...
    for itr in range(args.epoch):
        ...
        for _ in range(num_batches):
            ...
            try:
                if use_ms:
                    ...
                    # OPTIONAL: supervise only one channel
                    ...
                    # ------------------ PER-DIMENSION (CHANNEL-BALANCED) LOSS ------------------
                    # loss = mean_c ( sum((pred-tgt)^2 * mask) / sum(mask) )
                    diff2 = (pred_s - tgt_s) ** 2  # (B,L,C)
                    se_sum = (diff2 * mb).sum(dim=(0, 1))  # (C,)
                    cnt = mb.sum(dim=(0, 1))  # (C,)
                    avai = cnt > 0
                    loss = (se_sum[avai] / (cnt[avai] + 1e-8)).mean()
                    ...
                    from lib.utils import compute_all_losses  # SS path

                    # NOTE: SS path already uses per-dim/channel-balanced reduction in your lib.utils
        ...
        try:
            with torch.no_grad():
                if use_ms:
                    # VAL aggregate (per-dimension metrics)
                    val_logs = []
                    for _ in range(int(data_obj["n_val_batches"])):
                        ...
                        pred_s, tgt_s, msk_s = _slice_target_channel(pred, tgt, msk, args.target_channel)
                        val_logs.append(_masked_metrics_perdim(pred_s, tgt_s, msk_s))
                    ...
                    if np.isfinite(val_res["mse"]) and val_res["mse"] < best_val_mse:
                        ...
                        for _ in range(int(data_obj["n_test_batches"])):
                            ...
                            test_logs.append(_masked_metrics_perdim(pred_s, tgt_s, msk_s))
                    ...
                    from lib.utils import evaluation  # SS path
        ...
        sys_logger.info(
            f"SYSTEM | epoch={itr} time={epoch_time:.2f}s lr={lr_now:.6g}"
