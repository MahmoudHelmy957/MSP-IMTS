...
class MultiScaleTPatchGNN(nn.Module):
    ...
    def __init__(
        ...
        # fusion mode: "concat" (original behaviour) or "attn" (new, optional)
        self._fusion = fusion
        ...
        # --- NEW (for "attn" fusion) ---
        # These stay None and unused if fusion="concat"
    ...
    # ------------ NEW: helper for attention-over-scales fusion ------------
    ...
    def _fuse_attn(self, reps, device: torch.device) -> torch.Tensor:
        ...
        # weighted sum over scale dimension
    ...
    def forward(self, X_list, tt_list, mk_list, time_steps_to_predict):
        ...
        if self._fusion == "concat":
            ...
            # NEW OPTIONAL MODE: attention over scales
