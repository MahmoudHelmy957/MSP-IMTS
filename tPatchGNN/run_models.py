...
class ScaleAttentionFusion(nn.Module):
    ...
    def forward(self, H_list, M_list):
        """
        H_list: list[(B, M_k, L, D)] encoded per-scale features
        M_list: list[(B, M_k, L, D)] masks for those features
        returns fused (B, d_hid), attw (B, K)
        """
        logits, zs = [], []
        for Hk, Mk in zip(H_list, M_list):
            pooled = self.masked_gap(Hk, Mk)          # (B, D)
            z = torch.tanh(self.proj(pooled))         # (B, d_hid)
            zs.append(z)
